...
var choicesList = Array.from(Array(script.flavoursTextures.length - 2).keys()).map((i) => i + 2);
...
function Stop() {
  ...
  choicesList = Array.from(Array(script.flavoursTextures.length - 2).keys()).map((i) => i + 2);
...
function GenerateRandomFlavour() {
  ...
  selectedChoices = choicesList.slice(0, script.maxRound);
  print("Selected choices: " + selectedChoices);
}
...
class HeadPanel {
  constructor(parentObject, imageObject, isLeft, choiceIndex = 0) {
